...
namespace ScriptParser
{
	...
	public class XmlParser
	{
		...
		public static bool ParseXml(Logger i_Logger, string i_sXml, ref XElement io_xeRootElem)
		{
			bool			bRet = false;
			int				iCurrDepth = 0;
				...
				while (reader.Read())
				{
					switch (reader.NodeType)
					{
						...
						default :
						{
							...
							i_Logger.Log(Level.Warning, "ScriptParser.XmlParser read unexpected XmlNodeType: " + reader.NodeType.ToString());
						}
						break;
					}       
				}           
			...
			{
				bRet = false;
				//Console.Error.WriteLine("ERROR: ScriptParser.XmlParser.ParseXml() caught exception '{0}'.", e.ToString());
				i_Logger.Log(e);
			}
		...
		/// <returns></returns>
		public static bool GetXmlString(Logger i_Logger, string i_sURL, out string o_sXml)
		{
			WebRequest		wrReq;
			WebResponse		wrResp;
			...
			{
				bRet = false;
				//Console.Error.WriteLine("ERROR: ScriptParser.GetXmlString() caught exception '{0}'.", e.ToString());
				i_Logger.Log(e);
			}
