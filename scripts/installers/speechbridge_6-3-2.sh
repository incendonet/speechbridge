...
# Check dependencies
if [ "$UPGRADEABLE_VER0" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER1" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER2" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER3" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER4" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER5" != "$INSTALLED_VER" ]; then
	echo "Not running a version this installer can upgrade from."									>> $LOGFILE
	echo "----------------------------------------------"
	echo "Your system must be running SpeechBridge '$UPGRADEABLE_VER0' or '$UPGRADEABLE_VER1' or '$UPGRADEABLE_VER2' or '$UPGRADEABLE_VER3' or '$UPGRADEABLE_VER4' or '$UPGRADEABLE_VER5'"
...
mv -f speechbridge-$OLDVER4.tar.bz2 $SBINST
mv -f speechbridge-$OLDVER3.tar.bz2 $SBINST
...
if [ "$answer1" == "yes" ] || [ $INSTALLMODE = $SILENT ]; then
	...
	if [ "$INSTALLED_VER" != "$UPGRADEABLE_VER5" ]; then
		echo "The installation for $UPGRADEABLE_VER5 was unsucessful, aborting."					>> $LOGFILE
		echo "----------------------------------------------"
		echo "The installation for $UPGRADEABLE_VER5 was unsucessful."
		echo "Aborting the upgrade now."
		echo "----------------------------------------------"
		echo
		Cleanup
