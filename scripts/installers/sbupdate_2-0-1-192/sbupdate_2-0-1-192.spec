Summary: SpeechBridge 2.0.1 Update
Name: sbupdate
Version: 2.0.1
Release: 192
License: Incendonet
Group: Applications/Communications
Source: /opt/speechbridge/update/sbupdate_3-0-1-192.tar.gz
...
%description
This package will update your installation to SpeechBridge 2.0.1.192
...
mkdir -p %{BAK}/%{name}-%{version}.%{release}/SBConfig >> %{INSTLOG} 2>> %{INSTLOG}
...
mv -f /etc/init.d/speakd %{BAK}/%{name}-%{version}.%{release}/bin >> %{INSTLOG} 2>> %{INSTLOG}
...
mv -f %{BIN}/speakd.up /etc/init.d/speakd >> %{INSTLOG} 2>> %{INSTLOG}
...
mv -f %{BAK}/%{name}-%{version}.%{release}/bin/speakd /etc/init.d/ >> %{INSTLOG} 2>> %{INSTLOG}
...
rm -rf /etc/cron.hourly/speechbridgemon.cron >> %{INSTLOG} 2>> %{INSTLOG}
...
%attr(0550,speechbridge,speechbridge) %{BIN}/AudioRtr_C4_GenericSip.up
%attr(0550,speechbridge,speechbridge) %{BIN}/AudioRtr_C4_ShoretelSip.up
...
%attr(0550,speechbridge,speechbridge) %{BIN}/speakd.up
