...
# mv -f AAMain_Template_$OLDVERDASH_MISLABELED.vxml.xml $SBINST

if [ "$answer1" = "yes"  -o  $INSTALLMODE = $SILENT ]; then
    # if [ $INSTALLMODE != $SILENT ]; then
        # diff $SBVDOCS/AAMain_Template.vxml.xml $SBINST/AAMain_Template_$OLDVERDASH_ORIG.vxml.xml > /dev/null
        # TEMPLATESDIFFER=$?
    ...
    rpm -ivh --force --nodeps $SBINST/speechbridge-$THISVER-*.i686.rpm
...
# #    if [ "x$answer2" = "xyes"  -o  $INSTALLMODE = $SILENT ]; then
        # mv -f $SBVDOCS/AAMain_Template_$THISVERDASH.vxml.xml $SBVDOCS/AAMain_Template.vxml.xml
        # TEMPLATEUPGRADED=1
