...
# Check dependencies
if [ "$UPGRADEABLE_VER0" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER1" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER2" != "$INSTALLED_VER" ]; then
	echo "Not running a version this installer can upgrade from." >> $LOGFILE
	echo "----------------------------------------------"
	echo "Your system must be running SpeechBridge '$UPGRADEABLE_VER0' or '$UPGRADEABLE_VER1' or '$UPGRADEABLE_VER2'"
...
if [ "$answer1" == "yes" ] || [ $INSTALLMODE = $SILENT ]; then
	...
	if [ "$INSTALLED_VER" != "$UPGRADEABLE_VER2" ]; then
		echo "The installation for $UPGRADEABLE_VER2 was unsucessful, aborting." >> $LOGFILE
		echo "----------------------------------------------"
		echo "The installation for $UPGRADEABLE_VER2 was unsucessful."
		echo "Aborting the upgrade now."
		echo "----------------------------------------------"
		echo
		Cleanup
