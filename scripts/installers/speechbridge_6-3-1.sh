...
# Check dependencies
if [ "$UPGRADEABLE_VER0" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER1" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER2" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER3" != "$INSTALLED_VER" ] && [ "$UPGRADEABLE_VER4" != "$INSTALLED_VER" ]; then
...
if [ "$answer1" == "yes" ] || [ $INSTALLMODE = $SILENT ]; then
	...
	if [ "$INSTALLED_VER" != "$UPGRADEABLE_VER4" ]; then
		echo "The installation for $UPGRADEABLE_VER4 was unsucessful, aborting."					>> $LOGFILE
		echo "----------------------------------------------"
		echo "The installation for $UPGRADEABLE_VER4 was unsucessful."
		echo "Aborting the upgrade now."
		echo "----------------------------------------------"
		echo
		Cleanup
