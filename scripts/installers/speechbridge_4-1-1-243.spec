Summary: SpeechBridge 4.1.1 Installer
Name: speechbridge
Version: 4.1.1
Release: 243
License: Incendonet
...
%description
This package will update SpeechBridge to version 4.1.1
...
Obsoletes: sbupdate-2.0.2-42_09042, sbupdate-3.0.1-301, sbupdate-3.1.1-178, speechbridge-4.0.1-157
...
mv -f %{ISBCONF}/assets/docs/SpeechBridgeAdminGuide_4-1.pdf                  %{SBCONF}/assets/docs/SpeechBridgeAdminGuide_4-1.pdf >> %{INSTLOG} 2>> %{INSTLOG}
...
#mv -f %{IVDS}/AAMain_Template.vxml.xml              %{VDS}/AAMain_Template.vxml.xml >> %{INSTLOG} 2>> %{INSTLOG}
...
#rm -rf %{SBCONF}/ServerSettingsEmail.aspx                                  >> %{INSTLOG}2>> %{INSTLOG}
#rm -rf %{SBCONF}/Global.asax                                               >> %{INSTLOG}2>> %{INSTLOG}
...
/bin/echo "Updating configuration files..."                                 >> %{INSTLOG}
mv -f %{BIN}/SBLocalRM.exe.config %{BIN}/SBLocalRM.exe.config.ORIG         >> %{INSTLOG} 2>> %{INSTLOG}
#/bin/sed 's|"BinsToStart" value="|"BinsToStart" value="SBLauncher;|g' %{BIN}/SBLocalRM.exe.config.ORIG > %{BIN}/SBLocalRM.exe.config   2>> %{INSTLOG}
/bin/sed 's|AudioRtr|SBLauncher;AudioRtr|g' %{BIN}/SBLocalRM.exe.config.ORIG > %{BIN}/SBLocalRM.exe.config   2>> %{INSTLOG}
rm -f %{BIN}/SBLocalRM.exe.config.ORIG                                    >> %{INSTLOG} 2>> %{INSTLOG}
...
rm -rf %{SBCONF}/assets/docs/SpeechBridgeAdminGuide_4-1.pdf                         >> %{INSTLOG} 2>> %{INSTLOG}

rm -rf %{SBBASE}/config/SBUpdate_%{BUILDVERDASH}_pgsql.sql                          >> %{INSTLOG} 2>> %{INSTLOG}
rm -rf %{SBBASE}/config/SBUpdate_DD_pgsql.sql                                       >> %{INSTLOG} 2>> %{INSTLOG}
...
#%attr(0440,speechbridge,speechbridge) %{IVDS}/AAMain_Template.vxml.xml
