...
# and installs the en-GB (Great Britain) model.  Otherwise it adds the model
# to the directory of languages to load.

THISVER="6.4.1.16172"
THISLANGCODE="en-GB"
THISLANGNAME="English - Great Britain"
...
MODEL_THIS=$MODELS_BASE/OtherLanguages/$THISLANGCODE/BritishEnglish.model
MODELDIGITS_THIS=$MODELS_BASE/OtherLanguages/$THISLANGCODE/BritishEnglishDigits.model
...
if [ "$answer1" = "yes" ]; then
	...
	$MONOBIN/mono --config $SBBIN/sbdbutils.exe.config $SBBIN/sbdbutils.exe --run-sqlscript-commands $SBCONF/sblp_replace.sql               >> $LOGFILE 2>> $LOGFILE
...
/bin/sed "s:en-US:$THISLANGCODE:" < $VDS/ABNFBoolean_en-US.gram > $VDS/ABNFBoolean_$THISLANGCODE.gram			2>> $LOGFILE
/bin/sed "s:en-US:$THISLANGCODE:" < $VDS/ABNFDate_en-US.gram > $VDS/ABNFDate_$THISLANGCODE.gram					2>> $LOGFILE
/bin/sed "s:en-US:$THISLANGCODE:" < $VDS/ABNFDigits_en-US.gram > $VDS/ABNFDigits_$THISLANGCODE.gram				2>> $LOGFILE
...
/bin/chown speechbridge:speechbridge $SBCONF/sblp_${THISLANGCODE}_pgsql.sql			>> $LOGFILE 2>> $LOGFILE
/bin/chmod ug+r $VDS/*.gram															>> $LOGFILE 2>> $LOGFILE
/bin/chmod ug+r $SBCONF/sblp_${THISLANGCODE}_pgsql.sql								>> $LOGFILE 2>> $LOGFILE

echo "Run SQL script..."                                                           >> $LOGFILE 2>> $LOGFILE
$MONOBIN/mono --config $SBBIN/sbdbutils.exe.config $SBBIN/sbdbutils.exe --run-sqlscript-commands $SBCONF/sblp_${THISLANGCODE}_pgsql.sql         >> $LOGFILE 2>> $LOGFILE
